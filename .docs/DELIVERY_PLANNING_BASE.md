...
ESCENARIO EXTRA: Caso de 2 drivers tomen el mismo pedido:

¿Estaba el unlock en la planificación original?
No. En ITERATIONS_DELIVERY.md no aparece un escenario ni un endpoint de unlock. Solo se menciona:

locked_by_runner_id para evitar que dos runners tomen el mismo pedido
Riesgo de concurrencia y mitigación con SELECT FOR UPDATE SKIP LOCKED
¿Por qué se añadió el unlock?
El unlock es la contrapartida necesaria del bloqueo:

Concepto	En el doc	En la implementación
- Lock	Sí: evita que dos runners tomen el mismo pedido donde	Runner acepta → locked_by_runner_id = runner_id
- Unlock  No se menciona pero donde	Admin desbloquea → locked_by_runner_id = None
Si existe un lock, debe existir una forma de liberarlo cuando el lock queda “huérfano” o incorrecto.

¿En qué participa en el flujo?
El unlock cubre un escenario operativo no descrito en los 6 originales:
Escenario 7: “Runner bloqueado / abandonado”
- Runner A acepta el pedido → estado PENDIENTE_RECOJO, locked por A.
- Runner A: se va, se cae la app, se queda sin batería, toma el pedido equivocado, etc.
- El pedido sigue bloqueado por A.
- Otros runners no pueden tomarlo (409).
- Admin ejecuta unlock → locked_by_runner_id = None → otro runner puede aceptarlo.
Resumen
- ¿En el doc original?	No, no está explícito
- ¿Por qué existe?	Porque el lock requiere una forma de liberarlo en casos anómalos
- Rol en el flujo	Acción de recuperación operativa cuando el lock queda huérfano o incorrecto
Tiene sentido documentar este escenario en ITERATIONS_DELIVERY.md como Escenario 7: Desbloqueo por Admin para que quede alineado con la implementación.
